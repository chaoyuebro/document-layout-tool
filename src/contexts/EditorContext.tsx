...
interface Project {
  ...
  ai_cards: any;
  card_images: any;
  card_images_base64?: Record<number, string>; // 新增：存储Base64图片数据
  card_images_metadata?: Record<number, { mimeType: string; size: number; originalUrl?: string }>; // 新增：图片元数据
  updated_at: string;
}
...
interface EditorContextType {
  ...
  cardImagesBase64: Record<number, string>; // 新增：Base64图片数据
  setCardImagesBase64: (images: Record<number, string>) => void;
  cardImagesMetadata: Record<number, { mimeType: string; size: number; originalUrl?: string }>; // 新增：图片元数据
  setCardImagesMetadata: (metadata: Record<number, { mimeType: string; size: number; originalUrl?: string }>) => void;
}
...
export const EditorProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [cardImagesBase64, setCardImagesBase64] = useState<Record<number, string>>({}); // 新增
  const [cardImagesMetadata, setCardImagesMetadata] = useState<Record<number, { mimeType: string; size: number; originalUrl?: string }>>({}); // 新增
  ...
  const createNewProject = useCallback(() => {
    ...
    setAiCards(null);
    setCardImages({});
    setCardImagesBase64({}); // 新增
    setCardImagesMetadata({}); // 新增
  }, []);
  ...
  const loadProject = useCallback(async (id: string) => {
    try {
      ...
      if (data) {
        ...
        setCardImagesBase64(data.card_images_base64 || {}); // 新增
        setCardImagesMetadata(data.card_images_metadata || {}); // 新增
  ...
  const saveProject = useCallback(async (extraData?: Partial<Project>) => {
    ...
    try {
      const projectData = {
        ...
        card_images_base64: cardImagesBase64, // 新增：Base64图片数据
        card_images_metadata: cardImagesMetadata, // 新增：图片元数据
  ...
  const setSource = (source: string) => {
    ...
    // 当源码手动改变时，清空之前的 AI 排版缓存，回到实时切片模式
    if (aiCards) setAiCards(null);
  };
